...
logging.basicConfig(
    ...
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
        logging.FileHandler('export_log.txt', encoding="utf-8"),
...
# Xuất file jsonl vào folder output
db_name = "glamira"
coll_name = "summary"
...
logging.info(f"Bắt đầu xuất file summary_{file_idx}.jsonl")
...
logging.info(f"Hoàn thành ghi {write_count} docs vào {file_idx} file jsonl")

# Đẩy file jsonl lên Google Cloud Storage
project_id = "project-480502"
bucket_name = "quannh_bucket"
gcs_folder = "ingest_files"
output_dir = "./output"
credential_file = "/home/lanhuonglsls123/keys/credential.json"
credential = service_account.Credentials.from_service_account_file(credential_file)

storage_client = storage.Client(credentials=credential, project=project_id)
bucket = storage_client.bucket(bucket_name)

for root, dirs, files in os.walk(output_dir):
    for file_name in files:
        local_path = os.path.join(root, file_name)
        # GCS path sẽ giữ nguyên cấu trúc folder
        relative_path = os.path.relpath(local_path, output_dir)
        gcs_path = f"{gcs_folder}/{relative_path}"
        blob = bucket.blob(gcs_path)
        logging.info(f"Uploading file {file_name} → gs://{bucket}/{gcs_path}")
        blob.upload_from_filename(local_path)

logging.info("Hoàn thành upload toàn bộ file lên GCS.")